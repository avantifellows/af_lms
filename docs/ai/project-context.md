...
- **Coverage**: V8 coverage via `monocart-reporter` + `monocart-coverage-reports`
...
- `.github/workflows/e2e-coverage-comment.yml`: posts coverage table on PRs (reads committed `coverage/coverage-summary.json`)
...
- Playwright config + coverage: `playwright.config.ts` (monocart-reporter with V8 coverage config)
- Coverage PR comments: `.github/workflows/e2e-coverage-comment.yml`
