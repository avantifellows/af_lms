...
- `db-service` owns additive Ecto migrations, database constraints, Student-eligibility cleanup, and narrow machine APIs for Profile identity preflight, prompt/config registration, and idempotent Profile ingestion. It does not duplicate normal Holistic product CRUD APIs.
- `af_lms` owns authenticated product reads and writes directly through its existing Postgres transaction boundary. It also owns the one-time Historical Notes import, academic-year Mapping rollover, live Student Context resolution, progress queries and CSV, and regeneration-request records.
