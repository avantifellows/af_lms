...
- The Holistic Mentorship runtime allowlist contains canonical Main DB Program IDs `1` (`JNV CoE`), `74` (`Punjab CoE`), `94` (`Punjab Nodal`), `78` (`EMRS CoE`), `88` (`Uttarakhand CoE`), and `99` (`Maharashtra Coaching Test Prep`) and covers every School in those Programs; it does not hard-code School IDs or counts
...
- Grade 12 launch Mentees with no LMS Grade 11 Holistic history see only their real configured Grade 12 Phases, each using the existing plan-wide numbering for the displayed `Phase N`; a real Locked Phase remains disabled with no persisted Notes access until it is opened
...
- For a launch Grade 12 Student in Programs 1 or 78, non-empty imported **Historical Holistic Notes** take precedence over the Student Profile in the first real Phase; Students without any substantive migrated answer see their Profile instead of an empty historical record
- For those launch Grade 12 Students, **Historical Holistic Notes** remain the fallback context in later Phases until a newer applicable Phase has submitted Post-Session Notes; the guarded import resolves one selected Program at a time, keeps the fixed reviewed Program 1 baseline, and requires reviewed Program 78 dry-run counts before an EMRS apply. Programs 74, 94, 88, and 99 are runtime-supported but have no historical source import or inherited baseline.
