...
- Quiz MongoDB is the operational questionnaire source and the current BigQuery form-response tables are its derived projection; the v1 `etl-next` operator flow consumes that projection and publishes generated Profiles to Main Postgres through `db-service`, while `reporting` has no Holistic Mentorship production role
...
- The generation flow processes any real Student in the Holistic Program allowlist who submitted either configured Profile Form without requiring a Mentor-Mentee Mapping; test, unmatched, ambiguous, and out-of-scope identities are excluded before AI processing and reported with a safe reason
- The current BigQuery projection's selected per-Student response set is authoritative in v1; Profile generation does not distinguish or merge underlying Quiz attempts
...
- Generation provenance stores the canonical Student reference, source Form and AF Session IDs, BigQuery `inserted_at_timestamp` as warehouse load time, answer fingerprint, questionnaire schema hash, Prompt Version and hash, model ID, generation status and timestamps, and actor or run reason without storing raw answers or the rendered prompt
...
- A **Holistic Mentorship Admin** may open Phases out of sequence; LMS does not enforce sequential transitions, and the latest ordered Open Phase remains Active
- Returning the Active Phase to Locked before Mentor work exists automatically makes the latest remaining Open Phase for that Grade Active; if none remains Open, the Grade has no Active Phase
...
- A **Holistic Mentorship Admin** may reorder or insert Phases only among Locked future Phases that have never had Mentor work; once a Phase has been opened or used, its position in the Plan is fixed, although Admins may still open Phases out of sequence
...
- In v1, the first successful Post-Session Notes autosave containing a non-empty answer marks a **Holistic Phase** as started and freezes its Grade, title, sequence position, Phase Guidance, and Post-Session Questions; started Phases stay Open, while Active continues to follow the latest ordered Open Phase
- V1 Phase opening and return-to-Locked changes are immediate manual Admin actions with confirmation and actor/time audit; the authenticated email is the required actor snapshot and the canonical User ID is optional because LMS access may be permission-only; scheduled Phase opening is out of scope
...
- Pending, Skipped, and Completed are system-derived with no manual status control; a saved Notes draft remains Pending, only its authoring Teacher sees the Draft saved indicator, and only Notes submission creates Completed status
...
- Only an active Staff Management Teacher assigned to a School in a supported Holistic Program is eligible to be a **Holistic Mentor**
...
- Teacher self-unassignment requires confirmation but no approval, notification, or entered reason; each **Holistic Mentorship Admin** or **Admin** assignment, reassignment, or removal requires confirmation and a non-empty free-text audit reason
...
- An unfinished Notes draft and the fact that it has been saved are visible only to its author while they remain the current Mentor; Program Managers, Program Admins, Holistic Mentorship Admins, and Admins see Pending status until Notes are submitted
...
- For a launch Grade 12 Student in either supported Program, non-empty imported **Historical Holistic Notes** take precedence over the Student Profile in the first real Phase; Students without any substantive migrated answer see their Profile instead of an empty historical record
- For those launch Grade 12 Students, **Historical Holistic Notes** remain the fallback context in later Phases until a newer applicable Phase has submitted Post-Session Notes; the guarded import resolves one selected Program at a time, keeps the fixed reviewed Program 1 baseline, and requires reviewed Program 78 dry-run counts before an EMRS apply
