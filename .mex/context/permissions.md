...
- **Holistic Student and configuration reads:** `program_read` is only the program-workspace/list gate; it does not authorize an arbitrary Student or configuration resource. Profile-status reads use `mapped_student_read` with the exact Student, School, Program, and Academic Year, and scoped manager access verifies the Student belongs to that School context. Phase Plan reads use the Admin-only `phase_configuration_read` action; `phase_configure` remains the mutation gate.
