...
- **Holistic Mapping lifecycle:** Teachers may claim, confirm takeover, or remove only their own Program 1 Mappings. Teacher exit, relevant app/seat-role changes, and seat loss end affected active Mappings in the same LMS transaction; another eligible seat at the same School/Program preserves access. User deletion is blocked by any Holistic Mapping or authored Post-Session Notes history.
