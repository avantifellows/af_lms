...
- **School-scoped routes:** `authorizeSchoolAccess(udise, { requireEdit: true })` (`api-auth.ts`) 403s read-only callers; the success result also carries `readOnly` so list endpoints can hand the UI the same verdict (combined-report GET returns `can_generate: false, blocked_reason: "read_only"`). Report generation and retry are work, not reads — they pass `requireEdit`. Passcode users cannot be read-only.
...
- **Intervention flag routes:** use `authorizeInterventionFlags(session, schoolKey)` from `src/lib/intervention-flags.ts`. Rule: anyone who can see the School's Students (School scope + `students` view) sees their flags; raising, adding notes and resolving also need `students` edit, so `read_only` accounts only view; passcode logins are refused because a shared login cannot attribute a note. There is no feature key. The cross-school list at `/admin/intervention-flags` is `requireAdmin` only.
