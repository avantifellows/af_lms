...
## Accepted Next Change: Phone Registration Mode (Foundation + Scoped Correction Implemented; Activation Pending)

GitHub issue https://github.com/avantifellows/af_lms/issues/296 is the build-ready decision issue. AF LMS slices #297–#301 have implemented the dormant foundation plus the Add Student and Bulk Upload paths, mode-aware template download, and scoped existing-Student phone correction: a typed code registry with Phone and Approved modes, mode-resolved canonical fields, shared mode-aware validation, an explicit mode/version handshake on NVS DB Service writes, mode-aware Add/Bulk UI and API behavior, the exact HQ Phone workbook, strict Phone bulk schema detection, example-row removal before limits and writes, x14-safe workbook compaction, directly retryable 11-column Phone rejected CSVs, duplicate/conflict result presentation, server-side three-fact phone-cohort inference, strict NVS correction authorization, atomic correction payload/error handling, and Edit guidance while the active mode remains Approved. The coordinated AF LMS + DB Service change will later remove PEN, Grade 10 Roll Number, and Annual Family Income from NVS Add/Bulk, use a 10-digit 6-9-starting parent phone as Student ID in `EnableStudents`, keep Portal Student ID + Date of Birth login, and backfill missing identifiers without re-keying the phone cohort. Add, bulk, template, and per-student route tests exercise Phone mode through explicit test-mode injection while the active mode remains Approved; the Approved Add/Bulk/Edit behavior and workbooks stay the default until the coordinated activation slice and DB Service deployment. When HQ approves the reduced registration contract, the existing JNV NVS Add Student and Bulk Upload flow will use Phone Registration Mode in both services:
...
The planning branch is `grill/nvs-phone-registration-mode`; ADR 0006 records the permanent phone-identity trade-off. Phone mode is not active in production until the remaining bulk schema, coordinated DB Service correction, backfill, activation, and other coordinated DB Service slices land and deploy.
