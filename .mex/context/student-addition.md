...
- Bulk v1 uses an approved two-step `.xlsx`/`.csv` flow: selecting a file is inert; `Check file` performs LMS-only local parsing/validation and writes nothing; `Add students` re-runs the authoritative LMS checks and calls the existing DB Service create contract only for rows that remain ready. The final result is shown when returned; an unreadable or ambiguous response is an unknown outcome, so the roster is refreshed and no automatic retry is made. `Reset`/`Upload another` clears the selected file, check summary, result, and errors and allows the same file to be selected again. The check summary is read-only. No DB Service contract or database schema change is part of this flow; Add students writes only through the existing contract.
