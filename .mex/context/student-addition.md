...
- Bulk v1 uses an approved two-step `.xlsx`/`.csv` flow: selecting a file is inert; `Check file` performs LMS-only local parsing/validation and writes nothing; Phone-mode duplicate phones are rejected on every matching row during Check, including when a matching row has other validation errors; Add repeats the same check and excludes those rows. Named example rows do not participate in duplicate detection; `Add students` re-runs the authoritative LMS checks and calls the existing DB Service create contract only for rows that remain ready. The final result is shown when returned; an unreadable or ambiguous response is an unknown outcome, so the roster is refreshed and no automatic retry is made. `Reset`/`Upload another` clears the selected file, check summary, result, and errors and allows the same file to be selected again. The check summary is read-only. Excel workbooks using the built-in automatic encryption password are decrypted in memory before normal XLSX parsing; user-password and unsupported encryption return a clear error. Ordinary XLSX/CSV behavior is unchanged. No DB Service contract or database schema change is part of this flow; Add students writes only through the existing contract.
