...
- The school enrollment tab shows `Add Student` only when the shared Student Addition gate passes and the selected program is `PROGRAM_IDS.NVS`; the modal lives in `src/components/enrollment/AddStudentModal.tsx`.
