...
- Dropout accepts the opaque `student_pk_id` plus explicit NVS `program_id`, reuses the Centre-free existing-Student gate for role, feature, School/region, actor NVS scope, and current NVS Batch membership, derives actor/School/Program/period and Student ID or PEN server-side, and proxies LMS `POST /api/student/dropout` to the DB Service program-dropout contract at `PATCH /api/dropout`. Historical APAAR is never sent as the NVS mutation key, and upstream failures are reduced to safe operator-facing errors.
