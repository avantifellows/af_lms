...
- If a bulk upload response is unreadable, the client retries the same file once. This is safe because the DB Service create-only endpoint is idempotent: rows committed by the first request return `already_exists` on retry and are never overwritten. A successful retry refreshes the roster; two unreadable responses warn that some rows may have been processed instead of reporting a definite failure.
