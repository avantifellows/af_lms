...
- Dropout accepts the opaque `student_pk_id` plus an explicit `program_id`. NVS reuses the Centre-free existing-Student gate and derives Student ID or PEN server-side; other Programs retain their existing Centre-based Program dropout gate and Student ID or historical APAAR fallback. Both paths proxy LMS `POST /api/student/dropout` to the DB Service program-dropout contract at `PATCH /api/dropout`.
