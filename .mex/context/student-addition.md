...
- Existing-Student Edit uses the shared Student Addition existing-Student gate before proxying canonical changed fields to DB Service `PATCH /api/lms/students/:student_id/update-with-enrollments`. The gate requires one current School and a current NVS Batch plus actor Program, role, feature, and School scope checks, but does not query or require a Centre. The route filters locked and ownership fields, normalizes `Other`, CWSN/PWD category, CBSE/Others, Grade 11/12, and NDA values, derives actor/School/Program/enrollment context server-side, and safely maps upstream field errors. The modal keeps Student ID, PEN, G10 roll, and historical APAAR disabled even when blank, sends only changed fields (with CWSN/category as a pair), does not expose manual Batch selection, and refreshes the roster after success.
...
- Existing matches never update records. PEN or generated Student ID matches return `already_exists`; safe match context can include Student ID, PEN, and historical APAAR.
