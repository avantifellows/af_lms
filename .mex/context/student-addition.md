...
GitHub issue https://github.com/avantifellows/af_lms/issues/296 is the build-ready decision issue. AF LMS slices #297–#300 have implemented the dormant foundation plus the single Add Student path, mode-aware template download, and scoped existing-Student phone correction: a typed code registry with Phone and Approved modes, mode-resolved canonical fields, shared mode-aware validation, an explicit mode/version handshake on NVS DB Service writes, a mode-aware Add modal/success flow, the exact HQ Phone workbook, server-side three-fact phone-cohort inference, strict NVS correction authorization, atomic correction payload/error handling, and Edit guidance while the active mode remains Approved. The coordinated AF LMS + DB Service change will later remove PEN, Grade 10 Roll Number, and Annual Family Income from NVS Add/Bulk, use a 10-digit 6-9-starting parent phone as Student ID in `EnableStudents`, keep Portal Student ID + Date of Birth login, and backfill missing identifiers without re-keying the phone cohort. Add, template, and per-student route tests exercise Phone mode through explicit test-mode injection while the active mode remains Approved; the Approved Add/Edit behavior and workbook stay the default until the coordinated activation slice and DB Service deployment. When HQ approves the reduced registration contract, the existing JNV NVS Add Student and Bulk Upload flow will use Phone Registration Mode in both services:
