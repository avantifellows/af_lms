...
GitHub issue https://github.com/avantifellows/af_lms/issues/296 is the build-ready decision issue. AF LMS slices #297, #298, and #299 have implemented the dormant foundation plus the single Add Student path and mode-aware template download: a typed code registry with Phone and Approved modes, mode-resolved canonical fields, shared mode-aware validation, an explicit mode/version handshake on the existing NVS Add/Bulk DB Service proxy, a mode-aware Add modal/success flow, and an active-mode GET that serves the exact HQ Phone workbook without rewriting it. Add and template route tests exercise Phone mode through explicit test-mode injection while the active mode remains Approved; the Approved Add behavior and workbook stay the default until the coordinated activation slice and DB Service deployment. When HQ approves the reduced registration contract, the existing JNV NVS Add Student and Bulk Upload flow will use Phone Registration Mode in both services:
...
The planning branch is `grill/nvs-phone-registration-mode`; ADR 0006 records the permanent phone-identity trade-off. Phone mode is not active in production until the remaining bulk schema, correction, backfill, activation, and coordinated DB Service slices land and deploy.
