...
- NVS Phone Registration Mode is approved and planned in issue #296 on the grill/nvs-phone-registration-mode planning branch. AF LMS slices #297–#301 now provide the code-controlled mode registry, mode-aware canonical validation foundation, mode/version handshake, mode-aware Add/Bulk UI and API paths, exact HQ Phone workbook download, strict Phone bulk schema detection, example-row/x14-safe parsing, retry CSVs, duplicate/conflict result presentation, and scoped per-student phone correction: the server infers the phone cohort from current JNV NVS + `EnableStudents` membership and phone/Student ID equality, gates correction to scoped NVS actors, carries the atomic correction contract, maps duplicate/mismatch failures, and gives operators login-change guidance. The coordinated AF LMS + DB Service change will later remove PEN, Grade 10 Roll Number, and Annual Family Income from NVS Add/Bulk, use a 10-digit 6-9-starting parent phone as Student ID in `EnableStudents`, keep Portal Student ID + Date of Birth login, and backfill missing identifiers without re-keying the phone cohort. See `context/student-addition.md` and ADR 0006.
