...
- Holistic progress rows/counts/pagination, filter options, Academic Year Mapping history, CSV rows, and School assignment coverage consume one shared resolved School-scope SQL predicate. Admin and Holistic Mentorship Admin reads remain program-wide across the supported Program allowlist. Program Managers and Program Admins receive a read-only Students & Progress workspace only when their resolved School/region/Centre-seat scope intersects an assigned supported Program; empty scope hides navigation and fails closed, and these roles retain no Mapping, Notes, Phase, Profile, assignment-coverage, or Student-detail access.
