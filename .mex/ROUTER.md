...
- Centre Management now reads and writes independent Grade 11 and Grade 12 Centre Exam Track mappings from that fixed five-code vocabulary. Assignments are current-state rows in `centre_exam_tracks`; unassignment hard-deletes rows, the unique Centre + Grade + Track key makes reassignment idempotent, and Centre schema preflight covers the table. The legacy Centre Stream remains until its cleanup slice. Production still requires the matching db-service migration mirrored by `e2e/fixtures/migrations/20260807010000_add_centre_exam_tracks.sql`.
