...
- NVS Phone Registration Mode is approved and planned in issue #296 on the grill/nvs-phone-registration-mode planning branch, but is not implemented yet. The coordinated AF LMS + DB Service change removes PEN, Grade 10 Roll Number, and Annual Family Income from NVS Add/Bulk, uses a 10-digit 6-9-starting parent phone as Student ID in `EnableStudents`, serves the exact HQ workbook, keeps Portal Student ID + Date of Birth login, and later backfills missing identifiers without re-keying the phone cohort. See `context/student-addition.md` and ADR 0006.
