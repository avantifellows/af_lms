...
- Holistic Mentorship profile-status APIs bind reads to the exact Student, School, Program, and Academic Year; the generic workspace `program_read` gate cannot authorize Student or Phase Setup data, and Phase Plan reads remain Admin-only. Global Admins and Holistic Mentorship Admins marked `read_only` retain the Phase Setup view without any Phase Plan mutation controls.
