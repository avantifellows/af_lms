...
- Staff Management seat roles: subject seats (physics/chemistry/maths/biology + `subject_tbd`), `apc`, and the PM tiers (apm/pm/spm/ph). APC is a teacher record seated with role `apc` — not a subject, so it never writes `teacher.subject_id`; the roster Role field shows "APC" and Add User can create one directly (Type = APC, subject optional).
