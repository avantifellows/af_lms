...
- Curriculum Log resolves exactly one active physical Centre for the selected School + Program and derives Grade-specific Exam Track choices from that Centre's mappings. Missing/ambiguous Centres, unmapped Grades, and mapped Tracks without in-syllabus config fail closed with distinct UI states; Program and Grade changes refresh/prune Track choices, and create validation rejects stale or forged unmapped Tracks. A Track removed from Centre Management remains selectable as history-only while active logs exist, with create/edit/delete and Chapter Completion controls disabled. E2E fixture Centres/mappings live in `20260807020000_seed_curriculum_centre_exam_tracks.sql`.
