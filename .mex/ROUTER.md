...
- NVS Phone Registration Mode is approved and planned in issue #296 on the grill/nvs-phone-registration-mode planning branch. AF LMS slices #297, #298, and #299 now provide the code-controlled mode registry, mode-aware canonical validation foundation, mode/version handshake, mode-aware single Add Student UI/API path, and an active-mode template download that serves the exact HQ Phone workbook byte-for-byte while the active mode remains Approved; the coordinated AF LMS + DB Service change will later remove PEN, Grade 10 Roll Number, and Annual Family Income from NVS Add/Bulk, use a 10-digit 6-9-starting parent phone as Student ID in `EnableStudents`, keep Portal Student ID + Date of Birth login, and backfill missing identifiers without re-keying the phone cohort. See `context/student-addition.md` and ADR 0006.
