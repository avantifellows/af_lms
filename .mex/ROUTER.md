...
- NVS Phone Registration Mode is approved and planned in issue #296 on the grill/nvs-phone-registration-mode planning branch. AF LMS slice #297 now provides the code-controlled mode registry, mode-aware canonical validation foundation, and mode/version handshake for NVS Add/Bulk while the active mode remains Approved; the coordinated AF LMS + DB Service change will later remove PEN, Grade 10 Roll Number, and Annual Family Income from NVS Add/Bulk, use a 10-digit 6-9-starting parent phone as Student ID in `EnableStudents`, serve the exact HQ workbook, keep Portal Student ID + Date of Birth login, and backfill missing identifiers without re-keying the phone cohort. See `context/student-addition.md` and ADR 0006.
