...
describe("GET /api/quiz-sessions", () => {
  ...
  it("returns paginated quiz sessions scoped to the school's class batches", async () => {
    ...
    expect(mocks.mockQuery).toHaveBeenNthCalledWith(
      ...
      [["EnableStudents"], ["EnableStudents_11_Engg_A"], 11, 0, null, null, false]
    );
  ...
  it("filters to live sessions with ?status=live", async () => {
    const { GET } = await loadRouteModule();
    mocks.mockGetServerSession.mockResolvedValue(ADMIN_SESSION);
    mocks.mockQuery
      .mockResolvedValueOnce([
        { id: 11, name: "Class 11 Engg A", batch_id: "EnableStudents_11_Engg_A", parent_id: 5, program_id: 1 },
      ])
      .mockResolvedValueOnce([]);

    await GET(new NextRequest("http://localhost/api/quiz-sessions?schoolId=42&status=live"));
  ...
  it("filters to one paper with ?cmsTestId= or ?resourceId=", async () => {
    ...
    expect(sessionQueries[0][1].slice(-3)).toEqual(["4379", null, false]);
    expect(sessionQueries[1][1].slice(-3)).toEqual([null, "501", false]);
  });
