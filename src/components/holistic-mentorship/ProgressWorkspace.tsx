...
type Progress = "pending" | "completed" | "skipped" | "no_active_phase";
type Row = {
  studentId: number; studentName: string; externalStudentId: string | null; grade: 11 | 12;
  schoolName: string; schoolCode: string; mentorName: string; mentorEmail: string | null;
  phaseId: number | null; phaseNumber: number | null; phaseTitle: string | null;
  phaseState: "open" | "locked" | null; progress: Progress;
};
type Options = {
...
export default function ProgressWorkspace() {
  ...
  const params = useMemo(() => {
    const value = new URLSearchParams({ academic_year: academicYear, page: String(page), sort, direction });
    if (school) value.set("school_code", school);
    if (grade) value.set("grade", grade);
    if (mentor) value.set("mentor_user_id", mentor);
    if (phase) value.set("phase_id", phase);
    if (progress) value.set("progress", progress);
    if (search.trim()) value.set("search", search.trim());
    return value;
  }, [academicYear, direction, grade, mentor, page, phase, progress, school, search, sort]);
  ...
  const update = (setter: (value: string) => void) => (event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
  ...
  const counts = [
    ["Mapped", data.counts.totalMapped], ["Pending", data.counts.pending],
    ["Completed", data.counts.completed], ["Skipped", data.counts.skipped],
    ["No active phase", data.counts.noActivePhase],
  ];
  ...
  return (
    <div className="min-w-0 max-w-full space-y-5">
      ...
      <div className="space-y-3 border-y border-border py-4">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <select aria-label="Phase lens" className="min-h-11 rounded-md border border-border bg-bg px-3 text-sm" value={phase} onChange={update(setPhase)}>
            ...
            {data.options.phases.map((item) => <option key={item.id} value={item.id}>Phase {item.number}: {item.title} (Grade {item.grade})</option>)}
          </select>
          <select aria-label="Filter by School" className="min-h-11 rounded-md border border-border bg-bg px-3 text-sm" value={school} onChange={update(setSchool)}>
            <option value="">All Schools</option>
            {data.options.schools.map((item) => <option key={item.code} value={item.code}>{item.name}</option>)}
          </select>
          <select aria-label="Filter by Grade" className="min-h-11 rounded-md border border-border bg-bg px-3 text-sm" value={grade} onChange={update(setGrade)}>
            <option value="">All Grades</option><option value="11">Grade 11</option><option value="12">Grade 12</option>
          </select>
          <select aria-label="Filter by Mentor" className="min-h-11 rounded-md border border-border bg-bg px-3 text-sm" value={mentor} onChange={update(setMentor)}>
            <option value="">All Mentors</option>
            {data.options.mentors.map((item) => <option key={item.userId} value={item.userId}>{item.name}</option>)}
          </select>
          <select aria-label="Filter by Progress" className="min-h-11 rounded-md border border-border bg-bg px-3 text-sm" value={progress} onChange={update(setProgress)}>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-xs text-text-muted">
            <span>{data.refreshedAt ? `Last refreshed ${new Date(data.refreshedAt).toLocaleString()}` : "Not refreshed"}</span>
            <Button variant="icon" size="sm" aria-label="Refresh" onClick={() => setRefresh((value) => value + 1)} disabled={loading}>
              <RefreshCw aria-hidden="true" className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />
            </Button>
          </div>
          <div className="flex items-center gap-2">
            <select aria-label="Sort results" className="min-h-9 rounded-md border border-border bg-bg px-2 text-xs" value={sort} onChange={update(setSort)}>
            ...
            <button type="button" className="min-h-9 rounded-md border border-border px-3 text-xs" onClick={() => setDirection((value) => value === "asc" ? "desc" : "asc")}>{direction === "asc" ? "Ascending" : "Descending"}</button>
      ...
      <div aria-label="Student progress results" className="w-full min-w-0 max-w-full overflow-x-auto border-y border-border">
        <table className="w-full min-w-[900px] text-left text-sm">
          ...
          <tbody className="divide-y divide-border">
            {loading && data.rows.length === 0 ? <tr><td colSpan={7} className="px-3 py-12 text-center text-text-muted">Loading mapped Students...</td></tr> :
             data.rows.length === 0 ? <tr><td colSpan={7} className="px-3 py-12 text-center text-text-muted">No mapped Students match these filters.</td></tr> :
             data.rows.map((row) => <tr key={row.studentId} className="hover:bg-hover-bg/50">
               ...
               <td className="px-3 py-3"><Badge variant={row.progress === "completed" ? "success" : row.progress === "skipped" ? "warning" : "default"}>{row.progress.replaceAll("_", " ")}</Badge></td>
               <td className="px-3 py-3"><div className="flex justify-end gap-1">
                 <Button variant="icon" size="sm" aria-label={`Profile for ${row.studentName}`} onClick={() => setProfileStudent(row)}><Sparkles aria-hidden="true" className="h-4 w-4" /></Button>
      ...
      <div className="flex items-center justify-between text-sm">
        ...
        <div className="flex gap-2"><Button variant="secondary" size="sm" disabled={page <= 1} onClick={() => setPage((value) => value - 1)}>Previous</Button>
          <Button variant="secondary" size="sm" disabled={page >= totalPages} onClick={() => setPage((value) => value + 1)}>Next</Button></div>
      </div>
