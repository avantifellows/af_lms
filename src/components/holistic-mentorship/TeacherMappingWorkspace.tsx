...
export default function TeacherMappingWorkspace({
  ...
  const visible = view === "mentees"
    ? students.filter((student) => student.ownership?.mentorUserId === actorUserId)
    : students;
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row">
        <label className="relative min-w-0 flex-1">
          ...
          <input
            className="min-h-11 w-full rounded-md border border-border bg-bg pl-10 pr-3 text-sm"
            value={search}
        ...
        <select
          ...
          value={grade}
          onChange={(event) => setGrade(event.target.value as SavedFilters["grade"])}
        >
      ...
      {loading ? (
        <p className="py-12 text-center text-sm text-text-muted">Loading Students...</p>
      ) : visible.length === 0 ? (
        <p className="py-12 text-center text-sm font-medium text-text-muted">
          {view === "assign" ? "No eligible Students to show yet." : "No Mentees assigned yet."}
        </p>
      ) : (
        <div className="overflow-x-auto border-y border-border">
          <table className="w-full min-w-[640px] text-left text-sm">
            <thead className="bg-bg-card-alt text-xs uppercase text-text-muted">
              <tr>
            ...
            <tbody className="divide-y divide-border">
              {visible.map((student) => {
                const mine = student.ownership?.mentorUserId === actorUserId;
                return (
      ...
      {view === "assign" && selected.length > 0 && (
        <div className="flex justify-end">
          <button
            type="button"
            className="inline-flex min-h-11 items-center gap-2 rounded-md bg-accent px-4 font-semibold text-text-on-accent hover:bg-accent-hover disabled:opacity-50"
            disabled={busy}
            onClick={() => void assign()}
          >
            <UserPlus aria-hidden="true" className="h-4 w-4" /> Assign {selected.length} selected
          </button>
        </div>
      )}
    </div>
